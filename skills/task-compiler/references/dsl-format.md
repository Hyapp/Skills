...
validate:                     # 可选。分两层：环境预检 + IR 校验
  precheck:                   # 可选。wave 0 之前的统一环境检查
    command: python {plugin_dir}/check_env.py {session_dir}
  command: python {plugin_dir}/validate.py {session_dir}     # 可选。执行前的 IR 校验
...
| `validate.command` | 否 | **IR 校验**：执行前运行，exit 0 继续，exit ≠ 0 阻断。支持模板变量：`{plugin_dir}`、`{session_dir}` |
| `validate.precheck.command` | 否 | **环境预检**：在所有 wave 开始前运行，exit 0 继续，exit ≠ 0 整体中止。与 IR 无关，只查环境就绪情况（工具在 PATH、auth 有效、网络可达）。支持模板变量：`{plugin_dir}`、`{session_dir}` |

...
def prepare_plugin(node: dict, output_dir: Path, available_plugins: dict) -> dict:
    ...
    # Resolve input IR path
    input_ir = ""
    input_from = node.get("input_from", "")
    if input_from:
        src_result_path = output_dir / "nodes" / input_from / "result"
        if src_result_path.exists():
            try:
                src_data = json.loads(src_result_path.read_text(encoding="utf-8"))
                ir_path = src_data.get("ir_path") or src_data.get("result", "")
                if ir_path:
                    input_ir = ir_path if Path(ir_path).is_absolute() else str((output_dir / ir_path).resolve())
            except json.JSONDecodeError:
                input_ir = str(src_result_path.resolve())
        else:
            return {"status": "failed", "error": f"input_from node '{input_from}' has no result file"}
